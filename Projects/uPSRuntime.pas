...
interface
uses
  System.SysUtils, uPSUtils{$IFDEF DELPHI6UP}, System.variants{$ENDIF}{$IFDEF MACOS},uPSCMac{$ELSE}{$IFNDEF PS_NOIDISPATCH}{$IFDEF DELPHI3UP}, Winapi.ActiveX, Winapi.Windows{$ELSE}, Winapi.Ole2{$ENDIF}{$ENDIF}{$ENDIF};
...
implementation
uses
 System.TypInfo {$IFDEF DELPHI3UP}{$IFNDEF FPC}{$IFNDEF KYLIX} , System.Win.ComObj {$ENDIF}{$ENDIF}{$ENDIF}{$IFDEF PS_FPC_HAS_COM}, System.Win.ComObj{$ENDIF};
...
begin
  ...
  case Longint(p.Ext1) of
    0: Stack.SetAnsiString(-1, tbtstring(System.SysUtils.IntToStr(Stack.{$IFNDEF PS_NOINT64}GetInt64{$ELSE}GetInt{$ENDIF}(-2)))); // inttostr
...
{$IFNDEF PS_NOWIDESTRING}
      if Stack.GetItem(Stack.Count -2)^.FType.BaseType = btUnicodeString then
        Stack.SetUnicodeString(-1, System.SysUtils.Trim(Stack.GetUnicodestring(-2))) // Trim
      else if Stack.GetItem(Stack.Count -2)^.FType.BaseType = btWideString then
        Stack.SetWideString(-1, System.SysUtils.Trim(Stack.GetWideString(-2))) // Trim
      else
{$ENDIF}      
        Stack.SetAnsiString(-1, AnsiString(System.SysUtils.Trim(String(Stack.GetAnsiString(-2)))));// Trim
...
{$IFNDEF PS_NOINT64}
    ...
    40: Stack.SetAnsiString(-1, tbtstring(System.SysUtils.IntToStr(Stack.GetInt64(-2))));// Int64ToStr
